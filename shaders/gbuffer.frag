...
float terrainHeight(vec3 p, float sdfVal) {
...
void main() {
    ...
    float sdfAtHit = hit ? sampleSDF(posPlanet) : 0.0;
    float heightValue = hit ? terrainHeight(posPlanet, sdfAtHit) : -1.0;
    ...
    if (hit) {
        normalPlanet = computeNormal(posPlanet, sdfAtHit);
